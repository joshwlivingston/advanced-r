...
A second name, `y` was assigned to this object. So when `x` or `y` is modified, copy-on-modify takes place.
```{r}
y <- x
y[[4]] <- 4L
```
...
So when you modify a list, you can view how this effects copy-on-modify behavior using `lobstr::ref()`.

The first and fifth references are the lists themselves. The sixth references points to the new object, 4.
