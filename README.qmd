...
Per the type coercion hierarchy, this will be converted to a double,  `c(1, 0)`. 1's are coerced to `TRUE` (and 0 to `FALSE`), and vice a versa. 
...
Per the documentation for `==`, atomic vectors that are of different types are coerced prior to evaluation. So, the left hand of the equality, `1`, will be coereced to a character vector prior to evaluation. Once coerced, the call is `"1" == "1"`, which is clearly true.
...
- `is.atomic()` checks if the vecotr is of type "logical", "integer", "numeric", "complex", "character" or "raw"
