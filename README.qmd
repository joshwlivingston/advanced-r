...
<em>1. Explain the relationship between `a`, `b`, `c`, and `d` in the following code:*
```{r}
a <- 1:10
b <- a
c <- b
...
Because lists store refereneces to objects, a repeated list is smaller than one would imagine. This is because the reference occupies less space in memeory than the object itself.
...
Note that this optimization does *not* apply when modifyins a vector's length.  Here, `z` is assigned to a new object upon "adding" a fourth element to the vector `z`.
