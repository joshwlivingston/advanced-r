...
Copy-on-modify also applies when an object with one standalone reference is modified.mHere, `z` is assigned to a new object upon modification of the original `z`. We see this by looking at the location in memory before and after the modificaiton.
